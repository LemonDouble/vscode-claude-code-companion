...
훅은 이벤트의 stdin JSON(cwd 포함)을 그대로 파일로 저장하며, 1시간 지난 이벤트 파일은 스스로 정리한다. 입력 대기 알림의 한계: 입력을 이미 처리했어도 떠 있는 알림 토스트는 자동으로 사라지지 않는다 (VS Code API 제약).
...
code --install-extension vscode-claude-code-companion-0.7.3.vsix
```
